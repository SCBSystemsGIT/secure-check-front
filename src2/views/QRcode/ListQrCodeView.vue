...
const { requests, error, fetchRequestsByComp, fetchRequests } =
  useRequestsList();
  ...
  <div class="container">
    ...
    <div class="d-flex justify-content-center py-3">
      ...
      >
        ...
        <template #checkin_date="{ data }">
          <span
            v-if="
              formatDate(data?.visitor?.checkIns[0]?.check_out_time) ==
              'Invalid Date'
            "
          >
          </span>
          ...
          <span v-else>
            {{ formatDate(data?.visitor?.checkIns[0]?.check_in_time) }}
          </span>
        </template>
        ...
        <template #actions="{ data }">
          ...
          <ui-icon role="button" @click="show(data.id)" v-if="!data.confirmed">
            edit
          </ui-icon>
        </template>
